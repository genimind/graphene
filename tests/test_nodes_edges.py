...
edge_mapper_filename   = './resources/edge_mapper.json'
...
class TestNodesAndEdges(unittest.TestCase):
    ...
    def test_genNodesAndEdges(self):
        ...
        self.assertEqual(nx.number_of_edges(g), 10)
        # locate an edge
        self.assertTrue(g.has_node(key5))
        self.assertTrue(g.has_node(key3))
        self.assertTrue(key3 in g)
        self.assertTrue(g.has_edge(key5, key3))
        # check node data
        node_data = g.nodes[key5]
        self.assertTrue(node_data != {})
        # print('node_data:', node_data)
        # check edge data
        edge_data = g.get_edge_data(key5, key3)
        self.assertTrue(edge_data != {})
        # the graph is bidirectional
        self.assertTrue(g.has_edge(key3, key5))
        # print('edge:', g.edges[(key3, key5)])
